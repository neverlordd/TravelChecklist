...
export default function App() {
  ...
  const [layout, setLayout] = useState(() => localStorage.getItem('travel-layout') === 'grid' ? 'grid' : 'list')
  ...
  useEffect(() => { localStorage.setItem('travel-layout', layout) }, [layout])
  useEffect(() => {
  ...
  return <div className="mx-auto min-h-dvh w-full max-w-2xl px-3 pb-[calc(6.75rem+env(safe-area-inset-bottom))] pt-[calc(40px+env(safe-area-inset-top))] sm:px-6 sm:pt-[calc(2rem+env(safe-area-inset-top))]">
    {section === 'settings' ? <SettingsScreen countries={countries} username={username} darkMode={darkMode} onThemeChange={setDarkMode} onAdd={() => setCountryModal({})} onEdit={(country) => setCountryModal(country)} onDelete={async (country) => {
      ...
      <header className="mb-3 grid gap-3 sm:mb-5 sm:flex sm:items-center sm:justify-between">
        ...
        <div className={`flex items-center gap-2 ${section === 'favorites' ? 'justify-end' : ''}`}>{section === 'places' && <div className="grid min-w-0 flex-1 grid-cols-2 rounded-[20px] border border-white/80 bg-white/90 p-1 shadow-[0_4px_18px_rgba(23,33,31,0.06)] backdrop-blur dark:border-white/10 dark:bg-surface/90 sm:flex sm:flex-none"><button onClick={() => setStatus('active')} className={`min-h-10 px-4 py-2 text-xs font-bold transition ${status === 'active' ? 'bg-ink text-white shadow-sm dark:bg-mint-600' : 'text-muted'}`}>Активные</button><button onClick={() => setStatus('done')} className={`min-h-10 px-4 py-2 text-xs font-bold transition ${status === 'done' ? 'bg-ink text-white shadow-sm dark:bg-mint-600' : 'text-muted'}`}>Выполненные</button></div>}<button type="button" onClick={() => { setLayout((current) => current === 'list' ? 'grid' : 'list'); haptic() }} className="grid h-12 w-12 shrink-0 place-items-center rounded-full border border-white/80 bg-white/90 text-ink shadow-[0_4px_18px_rgba(23,33,31,0.06)] outline-none transition focus-visible:ring-4 focus-visible:ring-mint-200 active:scale-95 dark:border-white/10 dark:bg-surface/90 dark:focus-visible:ring-mint-500/30" aria-label={layout === 'list' ? 'Показать сеткой' : 'Показать списком'}>{layout === 'list' ? <LayoutGrid size={20} /> : <List size={21} />}</button></div>
      </header>

      <CategoryTabs value={category} onChange={setCategory} />
      {error && <ErrorBanner text={error} close={() => setError('')} />}
      {visibleItems.length ? <main className={layout === 'grid' ? 'grid grid-cols-2 items-stretch gap-2.5' : 'space-y-2.5'}>{visibleItems.map((item) => <PlaceCard key={item.id} grid={layout === 'grid'} item={item} country={countries.find((value) => value.id === item.country)} showCountry={section === 'favorites'} onOpen={() => setPreviewId(item.id)} onToggle={() => patchItem(item, { is_completed: !item.is_completed })} onFavorite={() => patchItem(item, { is_favorite: !item.is_favorite })} onRate={(rating) => patchItem(item, { rating })} onEdit={() => setItemModal(item)} onDelete={() => deleteItem(item)} />)}</main> : <EmptyState favorites={section === 'favorites'} completed={section === 'places' && status === 'done'} onAdd={() => setItemModal({})} />}
    </>}
...
function PlacePreview({ item, country, onClose, onEdit, onToggle, onFavorite, onRate }) {
  ...
  const mapUrl = item.maps_url || (item.latitude != null && item.longitude != null ? `https://www.google.com/maps/search/?api=1&query=${item.latitude},${item.longitude}` : null)
  return <Sheet title="О месте" onClose={close} footer={<div className="grid grid-cols-2 gap-2"><button onClick={onEdit} className={`flex min-h-14 items-center justify-center gap-2 rounded-[20px] bg-ink px-3 text-sm font-extrabold text-white shadow-[0_6px_18px_rgba(23,33,31,0.18)] dark:bg-white/10 sm:text-base ${mapUrl ? '' : 'col-span-2'}`}><Pencil size={19} /> Изменить</button>{mapUrl && <a href={mapUrl} target="_blank" rel="noreferrer" className="flex min-h-14 items-center justify-center gap-2 rounded-[20px] bg-mint-600 px-3 text-center text-sm font-extrabold text-white shadow-[0_6px_18px_rgba(124,58,237,0.25)] sm:text-base"><MapPin size={20} /> Открыть карту</a>}</div>}>
