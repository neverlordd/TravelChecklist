...
function EmptyState({ favorites, completed, onAdd }) {
  ...
  return <div className="empty-panel rounded-[30px] border border-white bg-white/95 px-6 py-14 text-center shadow-[0_6px_24px_rgba(23,33,31,0.055)] dark:border-white/10 dark:bg-surface/95"><div className="mx-auto mb-3 grid h-12 w-12 place-items-center rounded-full bg-mint-50 text-mint-700 dark:bg-mint-500/15 dark:text-mint-200">{favorites ? <Heart size={22} /> : completed ? <Check size={22} /> : <MapPin size={22} />}</div><h2 className="font-extrabold">{title}</h2><p className="mt-1 text-sm text-muted">{description}</p>{!favorites && !completed && <button onClick={onAdd} className="mt-4 rounded-full bg-mint-50 px-4 py-2.5 text-sm font-bold text-mint-700 dark:bg-mint-500/15 dark:text-mint-200">Добавить место</button>}</div>
}
