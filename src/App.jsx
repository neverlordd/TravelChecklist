...
function PlaceCard({ item, country, showCountry, onOpen, onToggle, onFavorite, onEdit, onDelete }) {
  ...
  return <article onClick={onOpen} className={`flex min-h-[104px] cursor-pointer gap-2.5 rounded-2xl border bg-white p-2 shadow-sm transition active:scale-[0.995] sm:gap-3 sm:p-2.5 ${item.is_completed ? 'border-mint-100 opacity-75' : 'border-slate-100'}`}>
    ...
    <div className="min-w-0 flex-1 py-0.5"><div className="flex items-start gap-1"><button onClick={stop(onToggle)} className={`mt-0.5 grid h-7 w-7 shrink-0 place-items-center rounded-lg border-2 ${item.is_completed ? 'border-mint-500 bg-mint-500 text-white' : 'border-slate-300 text-transparent'}`} aria-label="Выполнено"><Check size={15} strokeWidth={3} /></button><h2 className={`line-clamp-2 flex-1 pt-0.5 text-[13px] font-extrabold leading-[1.35] sm:text-sm ${item.is_completed ? 'line-through text-muted' : 'text-ink'}`}>{item.title}</h2><button onClick={stop(onFavorite)} className="grid h-8 w-8 shrink-0 place-items-center rounded-lg" aria-label="Избранное"><Heart size={19} className={item.is_favorite ? 'fill-rose-500 text-rose-500' : 'text-slate-300'} /></button></div>
      {item.description && <p className="mt-1 line-clamp-2 text-xs leading-relaxed text-muted">{item.description}</p>}
      <div className="mt-1.5 flex min-w-0 items-center gap-1"><span className="max-w-[4.5rem] truncate rounded-lg bg-slate-50 px-2 py-1.5 text-[10px] font-bold text-muted min-[380px]:max-w-[6.5rem]">{showCountry ? `${country?.emoji || '✈️'} ${country?.name || ''}` : item.category}</span>{mapUrl && <a href={mapUrl} target="_blank" rel="noreferrer" onClick={(event) => event.stopPropagation()} className="inline-flex min-h-8 shrink-0 items-center gap-1 rounded-lg bg-mint-50 px-2 text-[10px] font-extrabold text-mint-700"><MapPin size={12} /> Карта</a>}<span className="flex-1" /><button onClick={stop(onEdit)} className="grid h-8 w-8 shrink-0 place-items-center rounded-lg text-muted active:bg-slate-100" aria-label="Редактировать"><Pencil size={15} /></button><button onClick={stop(onDelete)} className="grid h-8 w-8 shrink-0 place-items-center rounded-lg text-slate-400 active:bg-red-50 active:text-red-500" aria-label="Удалить"><Trash2 size={15} /></button></div>
    </div>
  </article>
}
...
function PlacePreview({ item, country, onClose, onEdit, onToggle, onFavorite }) {
  ...
  const mapUrl = item.maps_url || (item.latitude != null && item.longitude != null ? `https://www.google.com/maps/search/?api=1&query=${item.latitude},${item.longitude}` : null)
  return <Sheet title="О месте" onClose={close} footer={<div className="grid grid-cols-2 gap-2">{mapUrl && <a href={mapUrl} target="_blank" rel="noreferrer" className="flex min-h-14 items-center justify-center gap-2 rounded-2xl bg-mint-600 px-3 text-center text-sm font-extrabold text-white sm:text-base"><MapPin size={20} /> Открыть карту</a>}<button onClick={onEdit} className={`flex min-h-14 items-center justify-center gap-2 rounded-2xl bg-ink px-3 text-sm font-extrabold text-white sm:text-base ${mapUrl ? '' : 'col-span-2'}`}><Pencil size={19} /> Изменить</button></div>}>
    {item.photo_url ? <img src={item.photo_url} alt={item.title} className="mb-4 h-52 w-full rounded-3xl object-cover sm:h-64" /> : <div className="mb-4 grid h-40 w-full place-items-center rounded-3xl bg-mint-50 text-mint-600"><MapPin size={38} /></div>}
    ...
    <div className="mt-5 grid grid-cols-2 gap-2"><button onClick={onToggle} className={`flex min-h-12 items-center justify-center gap-2 rounded-2xl text-sm font-extrabold ${item.is_completed ? 'bg-mint-600 text-white' : 'bg-white text-muted shadow-sm'}`}><Check size={18} /> {item.is_completed ? 'Выполнено' : 'Отметить'}</button><button onClick={onFavorite} className={`flex min-h-12 items-center justify-center gap-2 rounded-2xl text-sm font-extrabold ${item.is_favorite ? 'bg-rose-50 text-rose-600' : 'bg-white text-muted shadow-sm'}`}><Heart size={18} className={item.is_favorite ? 'fill-current' : ''} /> {item.is_favorite ? 'Любимое' : 'В избранное'}</button></div>
    {item.external_url && <a href={item.external_url} target="_blank" rel="noreferrer" className="mt-3 flex min-h-12 items-center justify-center gap-2 rounded-2xl border border-slate-200 bg-white text-sm font-extrabold text-ink"><ExternalLink size={18} /> Открыть сайт</a>}
  </Sheet>
}
...
function ItemSheet({ item, countryId, username, onClose, onSave, onError }) {
  ...
  return <Sheet title={item ? 'Редактировать место' : 'Новое место'} onClose={close} footer={<button type="submit" form="place-form" disabled={saving} className="flex min-h-14 w-full items-center justify-center gap-2 rounded-2xl bg-ink px-4 font-extrabold text-white shadow-xl disabled:opacity-60">{saving ? <LoaderCircle className="animate-spin" size={20} /> : <Check size={20} />} Сохранить</button>}>
    <form id="place-form" onSubmit={submit} className="space-y-4 pb-1 sm:space-y-5">
      <section className="space-y-3 rounded-3xl border border-mint-100 bg-mint-50/60 p-3"><div><h2 className="text-sm font-extrabold text-ink">Быстрое заполнение</h2><p className="mt-0.5 text-xs leading-relaxed text-muted">Вставьте ссылку — название, описание и фото загрузятся сами.</p></div>
...
function Sheet({ title, onClose, children, footer }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-900/35 backdrop-blur-sm sm:items-center sm:p-4" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className="flex max-h-[calc(100dvh-env(safe-area-inset-top)-8px)] w-full max-w-lg flex-col overflow-hidden rounded-t-[26px] bg-canvas shadow-2xl sm:max-h-[94dvh] sm:rounded-[28px]"><div className="mx-auto mt-2 h-1 w-10 shrink-0 rounded-full bg-slate-300 sm:hidden" /><header className="flex shrink-0 items-center justify-between px-4 py-3 sm:px-5 sm:py-4"><h1 className="text-lg font-extrabold sm:text-xl">{title}</h1><button onClick={onClose} className="icon-button" aria-label="Закрыть"><X size={21} /></button></header><div className="min-h-0 flex-1 overscroll-contain overflow-y-auto px-4 pb-4 sm:px-5 sm:pb-5">{children}</div>{footer && <footer className="shrink-0 border-t border-slate-100 bg-white/95 px-4 pb-[calc(0.75rem+env(safe-area-inset-bottom))] pt-3 backdrop-blur sm:px-5 sm:pb-4">{footer}</footer>}</section></div>
}
