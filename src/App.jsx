...
function PlaceCard({ item, country, showCountry, onOpen, onToggle, onFavorite, onRate, onEdit, onDelete }) {
  ...
  return <article onClick={onOpen} className={`cursor-pointer rounded-[28px] border bg-white/95 p-2.5 shadow-[0_5px_22px_rgba(23,33,31,0.055)] transition active:scale-[0.995] sm:p-3 ${item.is_completed ? 'border-mint-100' : 'border-white'}`}>
    <div className="flex min-h-24 items-stretch gap-3">
      <div className="relative h-24 w-24 shrink-0 overflow-hidden rounded-[22px] bg-mint-50">{item.photo_url ? <img src={item.photo_url} alt="" className="h-full w-full object-cover" loading="lazy" /> : <div className="grid h-full w-full place-items-center text-mint-600"><MapPin size={26} /></div>}<span className="absolute inset-x-1.5 bottom-1.5 truncate rounded-full bg-slate-900/65 px-2 py-1 text-center text-[10px] font-extrabold text-white backdrop-blur">{item.category}</span></div>
      <div className="flex min-w-0 flex-1 flex-col justify-center py-1"><h2 className={`line-clamp-2 text-[15px] font-extrabold leading-snug sm:text-base ${item.is_completed ? 'line-through text-muted' : 'text-ink'}`}>{item.title}</h2>{item.description && <p className="mt-1.5 line-clamp-2 text-xs leading-relaxed text-muted sm:text-[13px]">{item.description}</p>}{showCountry && <span className="mt-2 w-fit max-w-full truncate rounded-full bg-slate-50 px-2.5 py-1 text-[10px] font-bold text-muted">{country?.emoji || '✈️'} {country?.name || ''}</span>}</div>
    </div>
    <footer className="mt-2.5 border-t border-slate-100 pt-2.5">{item.is_completed && <StarRating value={item.rating} onChange={onRate} compact />}<div className={`flex items-center gap-1.5 ${item.is_completed ? 'mt-1.5' : ''}`}>{mapUrl ? <a href={mapUrl} target="_blank" rel="noreferrer" onClick={(event) => event.stopPropagation()} className="flex h-11 min-w-0 flex-1 items-center justify-center gap-1.5 rounded-full border border-slate-100 bg-white px-2 text-xs font-extrabold text-ink shadow-[0_2px_10px_rgba(23,33,31,0.05)]"><MapPin size={17} /><span className="max-[340px]:sr-only">Карта</span></a> : <button onClick={stop(onOpen)} className="flex h-11 min-w-0 flex-1 items-center justify-center gap-1.5 border border-slate-100 bg-white px-2 text-xs font-extrabold text-ink shadow-[0_2px_10px_rgba(23,33,31,0.05)]"><MapPin size={17} /><span className="max-[340px]:sr-only">О месте</span></button>}<button onClick={stop(onToggle)} className={`grid h-11 w-11 shrink-0 place-items-center border shadow-[0_2px_10px_rgba(23,33,31,0.05)] ${item.is_completed ? 'border-mint-500 bg-mint-500 text-white' : 'border-slate-100 bg-white text-ink'}`} aria-label={item.is_completed ? 'Вернуть в активные' : 'Отметить выполненным'}><Check size={19} strokeWidth={2.5} /></button><button onClick={stop(onEdit)} className="grid h-11 w-11 shrink-0 place-items-center border border-slate-100 bg-white text-ink shadow-[0_2px_10px_rgba(23,33,31,0.05)]" aria-label="Редактировать"><Pencil size={18} /></button><button onClick={stop(onFavorite)} className="grid h-11 w-11 shrink-0 place-items-center border border-slate-100 bg-white shadow-[0_2px_10px_rgba(23,33,31,0.05)]" aria-label="Избранное"><Heart size={20} className={item.is_favorite ? 'fill-rose-500 text-rose-500' : 'text-slate-500'} /></button><button onClick={stop(onDelete)} className="grid h-11 w-11 shrink-0 place-items-center border border-slate-100 bg-white text-slate-600 shadow-[0_2px_10px_rgba(23,33,31,0.05)] active:bg-red-50 active:text-red-500" aria-label="Удалить"><Trash2 size={18} /></button></div></footer>
  </article>
}

function StarRating({ value = 0, onChange, compact = false }) {
  return <div className={`flex items-center ${compact ? 'justify-center gap-0.5' : 'gap-1'}`} role="group" aria-label="Оценка места">{[1, 2, 3, 4, 5].map((star) => <button key={star} type="button" onClick={(event) => { event.stopPropagation(); onChange(value === star ? null : star); haptic() }} className={`grid shrink-0 place-items-center rounded-full active:scale-90 ${compact ? 'h-7 w-7' : 'h-10 w-10'}`} aria-label={`${star} из 5`}><Star size={compact ? 18 : 26} strokeWidth={2} className={star <= (value || 0) ? 'fill-amber-400 text-amber-400' : 'text-slate-300'} /></button>)}</div>
}
