...
import {
  ...
  LockKeyhole, Map, MapPin, Moon, Pencil, Plus, Settings, Star, Sun, Trash2, X,
} from 'lucide-react'
...
export default function App() {
  ...
  const [darkMode, setDarkMode] = useState(() => {
    ...
    return window.Telegram?.WebApp?.colorScheme === 'dark' || window.matchMedia?.('(prefers-color-scheme: dark)').matches
  })
  ...
  useEffect(() => {
    ...
    document.querySelector('meta[name="theme-color"]')?.setAttribute('content', darkMode ? '#0E0B16' : '#F6F5F9')
    const app = window.Telegram?.WebApp
    app?.setHeaderColor?.(darkMode ? '#0E0B16' : '#F6F5F9')
    app?.setBackgroundColor?.(darkMode ? '#0E0B16' : '#F6F5F9')
  }, [darkMode])
  ...
  return <div className="mx-auto min-h-dvh w-full max-w-2xl px-3 pb-[calc(6.75rem+env(safe-area-inset-bottom))] pt-[calc(40px+env(safe-area-inset-top))] sm:px-6 sm:pt-[calc(2rem+env(safe-area-inset-top))]">
    {section === 'settings' ? <SettingsScreen countries={countries} username={username} darkMode={darkMode} onThemeChange={setDarkMode} onAdd={() => setCountryModal({})} onEdit={(country) => setCountryModal(country)} onDelete={async (country) => {
      ...
      <header className="mb-3 grid gap-3 sm:mb-5 sm:flex sm:items-center sm:justify-between">
        ...
        {section === 'places' && <div className="grid w-full grid-cols-2 rounded-[20px] border border-white/80 bg-white/90 p-1 shadow-[0_4px_18px_rgba(23,33,31,0.06)] backdrop-blur dark:border-white/10 dark:bg-surface/90 sm:flex sm:w-auto"><button onClick={() => setStatus('active')} className={`min-h-10 px-4 py-2 text-xs font-bold transition ${status === 'active' ? 'bg-ink text-white shadow-sm dark:bg-mint-600' : 'text-muted'}`}>Активные</button><button onClick={() => setStatus('done')} className={`min-h-10 px-4 py-2 text-xs font-bold transition ${status === 'done' ? 'bg-ink text-white shadow-sm dark:bg-mint-600' : 'text-muted'}`}>Выполненные</button></div>}
      </header>
    ...
    {section !== 'settings' && countries.length > 0 && <button onClick={() => setItemModal({})} className="fixed bottom-[calc(6rem+env(safe-area-inset-bottom))] right-4 z-20 grid h-14 w-14 place-items-center rounded-full border border-white/20 bg-mint-600 text-white shadow-[0_10px_28px_rgba(124,58,237,0.32)] transition active:scale-95 sm:right-6" aria-label="Добавить место"><Plus size={25} /></button>}
...
function PlaceCard({ item, country, showCountry, onOpen, onToggle, onFavorite, onRate, onEdit, onDelete }) {
  ...
  return <article onClick={onOpen} className={`cursor-pointer rounded-[28px] border bg-white/95 p-2.5 shadow-[0_5px_22px_rgba(23,33,31,0.055)] transition active:scale-[0.995] dark:bg-surface/95 dark:shadow-[0_8px_28px_rgba(0,0,0,0.22)] sm:p-3 ${item.is_completed ? 'border-mint-100 dark:border-mint-500/35' : 'border-white dark:border-white/10'}`}>
    <div className="flex min-h-24 items-stretch gap-3">
      <div className="relative h-24 w-24 shrink-0 overflow-hidden rounded-[22px] bg-mint-50 dark:bg-mint-500/15">{item.photo_url ? <img src={item.photo_url} alt="" className="h-full w-full object-cover" loading="lazy" /> : <div className="grid h-full w-full place-items-center text-mint-600 dark:text-mint-200"><MapPin size={26} /></div>}<span className="absolute inset-x-1.5 bottom-1.5 truncate rounded-full bg-slate-900/65 px-2 py-1 text-center text-[10px] font-extrabold text-white backdrop-blur">{item.category}</span></div>
      <div className="flex min-w-0 flex-1 flex-col justify-center py-1"><h2 className={`line-clamp-2 text-[15px] font-extrabold leading-snug sm:text-base ${item.is_completed ? 'line-through text-muted' : 'text-ink'}`}>{item.title}</h2>{item.description && <p className="mt-1.5 line-clamp-2 text-xs leading-relaxed text-muted sm:text-[13px]">{item.description}</p>}{showCountry && <span className="mt-2 w-fit max-w-full truncate rounded-full bg-slate-50 px-2.5 py-1 text-[10px] font-bold text-muted dark:bg-white/5">{country?.emoji || '✈️'} {country?.name || ''}</span>}</div>
    </div>
    <footer className="mt-2.5 border-t border-slate-100 pt-2.5 dark:border-white/10">{item.is_completed && <StarRating value={item.rating} onChange={onRate} compact />}<div className={`flex items-center gap-1.5 ${item.is_completed ? 'mt-1.5' : ''}`}>{mapUrl ? <a href={mapUrl} target="_blank" rel="noreferrer" onClick={(event) => event.stopPropagation()} className="flex h-11 min-w-0 flex-1 items-center justify-center gap-1.5 rounded-full border border-slate-100 bg-white px-2 text-xs font-extrabold text-ink shadow-[0_2px_10px_rgba(23,33,31,0.05)] dark:border-white/10 dark:bg-white/5"><MapPin size={17} /><span className="max-[340px]:sr-only">Карта</span></a> : <button onClick={stop(onOpen)} className="flex h-11 min-w-0 flex-1 items-center justify-center gap-1.5 border border-slate-100 bg-white px-2 text-xs font-extrabold text-ink shadow-[0_2px_10px_rgba(23,33,31,0.05)] dark:border-white/10 dark:bg-white/5"><MapPin size={17} /><span className="max-[340px]:sr-only">О месте</span></button>}<button onClick={stop(onToggle)} className={`grid h-11 w-11 shrink-0 place-items-center border shadow-[0_2px_10px_rgba(23,33,31,0.05)] ${item.is_completed ? 'border-mint-500 bg-mint-500 text-white' : 'border-slate-100 bg-white text-ink dark:border-white/10 dark:bg-white/5'}`} aria-label={item.is_completed ? 'Вернуть в активные' : 'Отметить выполненным'}><Check size={19} strokeWidth={2.5} /></button><button onClick={stop(onEdit)} className="grid h-11 w-11 shrink-0 place-items-center border border-slate-100 bg-white text-ink shadow-[0_2px_10px_rgba(23,33,31,0.05)] dark:border-white/10 dark:bg-white/5" aria-label="Редактировать"><Pencil size={18} /></button><button onClick={stop(onFavorite)} className="grid h-11 w-11 shrink-0 place-items-center border border-slate-100 bg-white shadow-[0_2px_10px_rgba(23,33,31,0.05)] dark:border-white/10 dark:bg-white/5" aria-label="Избранное"><Heart size={20} className={item.is_favorite ? 'fill-rose-500 text-rose-500' : 'text-slate-500 dark:text-slate-400'} /></button><button onClick={stop(onDelete)} className="grid h-11 w-11 shrink-0 place-items-center border border-slate-100 bg-white text-slate-600 shadow-[0_2px_10px_rgba(23,33,31,0.05)] active:bg-red-50 active:text-red-500 dark:border-white/10 dark:bg-white/5 dark:text-slate-300" aria-label="Удалить"><Trash2 size={18} /></button></div></footer>
  </article>
}
...
function PlacePreview({ item, country, onClose, onEdit, onToggle, onFavorite, onRate }) {
  ...
  const mapUrl = item.maps_url || (item.latitude != null && item.longitude != null ? `https://www.google.com/maps/search/?api=1&query=${item.latitude},${item.longitude}` : null)
  return <Sheet title="О месте" onClose={close} footer={<div className="grid grid-cols-2 gap-2">{mapUrl && <a href={mapUrl} target="_blank" rel="noreferrer" className="flex min-h-14 items-center justify-center gap-2 rounded-[20px] bg-mint-600 px-3 text-center text-sm font-extrabold text-white shadow-[0_6px_18px_rgba(124,58,237,0.25)] sm:text-base"><MapPin size={20} /> Открыть карту</a>}<button onClick={onEdit} className={`flex min-h-14 items-center justify-center gap-2 rounded-[20px] bg-ink px-3 text-sm font-extrabold text-white shadow-[0_6px_18px_rgba(23,33,31,0.18)] dark:bg-white/10 sm:text-base ${mapUrl ? '' : 'col-span-2'}`}><Pencil size={19} /> Изменить</button></div>}>
    {item.photo_url ? <img src={item.photo_url} alt={item.title} className="mb-4 h-52 w-full rounded-[28px] object-cover sm:h-64" /> : <div className="mb-4 grid h-40 w-full place-items-center rounded-[28px] bg-mint-50 text-mint-600 dark:bg-mint-500/15 dark:text-mint-200"><MapPin size={38} /></div>}
    <div className="mb-3 flex flex-wrap items-center gap-2"><span className="rounded-full bg-mint-50 px-3 py-1.5 text-xs font-extrabold text-mint-700 dark:bg-mint-500/15 dark:text-mint-200">{item.category}</span><span className="rounded-full bg-white px-3 py-1.5 text-xs font-bold text-muted dark:bg-white/5">{country?.emoji || '✈️'} {country?.name || ''}</span></div>
    ...
    {item.is_completed && <div className="mt-5 rounded-[22px] bg-white/95 p-3 shadow-[0_3px_14px_rgba(23,33,31,0.06)] dark:bg-white/5"><p className="mb-1 text-xs font-extrabold text-muted">Ваша оценка</p><StarRating value={item.rating} onChange={onRate} /></div>}
    <div className="mt-5 grid grid-cols-2 gap-2"><button onClick={onToggle} className={`flex min-h-12 items-center justify-center gap-2 rounded-[20px] text-sm font-extrabold ${item.is_completed ? 'bg-mint-600 text-white' : 'bg-white/95 text-muted shadow-[0_3px_14px_rgba(23,33,31,0.06)] dark:bg-white/5'}`}><Check size={18} /> {item.is_completed ? 'Выполнено' : 'Отметить'}</button><button onClick={onFavorite} className={`flex min-h-12 items-center justify-center gap-2 rounded-[20px] text-sm font-extrabold ${item.is_favorite ? 'bg-rose-50 text-rose-600 dark:bg-rose-500/15 dark:text-rose-300' : 'bg-white/95 text-muted shadow-[0_3px_14px_rgba(23,33,31,0.06)] dark:bg-white/5'}`}><Heart size={18} className={item.is_favorite ? 'fill-current' : ''} /> {item.is_favorite ? 'Любимое' : 'В избранное'}</button></div>
    {item.external_url && <a href={item.external_url} target="_blank" rel="noreferrer" className="mt-3 flex min-h-12 items-center justify-center gap-2 rounded-[20px] border border-white bg-white/95 text-sm font-extrabold text-ink shadow-[0_3px_14px_rgba(23,33,31,0.06)] dark:border-white/10 dark:bg-white/5"><ExternalLink size={18} /> Открыть сайт</a>}
  </Sheet>
}
...
function BottomNav({ value, onChange }) {
  ...
  return <nav className="fixed inset-x-3 bottom-[calc(0.5rem+env(safe-area-inset-bottom))] z-30 mx-auto max-w-[40rem] rounded-[28px] border border-white/80 bg-white/[0.88] px-2 shadow-[0_10px_35px_rgba(23,33,31,0.12)] backdrop-blur-xl dark:border-white/10 dark:bg-surface/[0.9] dark:shadow-[0_12px_38px_rgba(0,0,0,0.35)]"><div className="mx-auto grid h-[66px] grid-cols-3 gap-1">{links.map(({ id, label, icon }) => <button key={id} onClick={() => onChange(id)} className={`my-1.5 flex min-w-0 flex-col items-center justify-center gap-1 rounded-[21px] text-[11px] font-bold transition active:scale-[0.97] ${value === id ? 'bg-mint-50 text-mint-700 shadow-inner dark:bg-mint-500/15 dark:text-mint-200' : 'text-slate-400 dark:text-slate-500'}`}>{icon}{label}</button>)}</div></nav>
}

function SettingsScreen({ countries, username, darkMode, onThemeChange, onAdd, onEdit, onDelete }) {
  return <main><header className="mb-5 sm:mb-6"><h1 className="text-[27px] font-extrabold tracking-tight sm:text-[28px]">Настройки</h1><p className="mt-1 text-sm text-muted">@{username}</p></header><div className="space-y-3"><section className="flex items-center gap-3 rounded-[28px] border border-white bg-white/95 p-3 shadow-[0_6px_24px_rgba(23,33,31,0.055)] dark:border-white/10 dark:bg-surface/95 sm:p-4"><div className="grid h-12 w-12 shrink-0 place-items-center rounded-full bg-mint-50 text-mint-700 dark:bg-mint-500/15 dark:text-mint-200">{darkMode ? <Moon size={21} /> : <Sun size={21} />}</div><div className="min-w-0 flex-1"><h2 className="font-extrabold">Тёмная тема</h2><p className="text-xs text-muted">Комфортный вид в вечернее время</p></div><button type="button" role="switch" aria-checked={darkMode} onClick={() => { onThemeChange(!darkMode); haptic() }} className={`relative h-8 w-[52px] shrink-0 outline-none transition focus-visible:ring-4 focus-visible:ring-mint-200 dark:focus-visible:ring-mint-500/30 ${darkMode ? 'bg-mint-600' : 'bg-slate-200 dark:bg-white/15'}`} aria-label="Переключить тёмную тему"><span className={`absolute top-1 grid h-6 w-6 place-items-center rounded-full bg-white shadow-sm transition-transform ${darkMode ? 'translate-x-6' : 'translate-x-1'}`} /></button></section><section className="rounded-[28px] border border-white bg-white/95 p-3 shadow-[0_6px_24px_rgba(23,33,31,0.055)] dark:border-white/10 dark:bg-surface/95 sm:p-4"><div className="mb-3 flex items-center justify-between gap-3"><div className="min-w-0"><h2 className="font-extrabold">Страны</h2><p className="text-xs text-muted">Добавляйте направления поездок</p></div><button onClick={onAdd} className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-mint-50 text-mint-700 active:scale-95 dark:bg-mint-500/15 dark:text-mint-200" aria-label="Добавить страну"><Plus size={20} /></button></div><div className="space-y-2">{countries.map((country) => <div key={country.id} className="flex min-h-14 items-center gap-2 rounded-[20px] bg-canvas px-3 py-2"><span className="text-2xl">{country.emoji}</span><span className="min-w-0 flex-1 truncate font-bold">{country.name}</span><button onClick={() => onEdit(country)} className="icon-button shrink-0" aria-label={`Редактировать ${country.name}`}><Pencil size={17} /></button><button onClick={() => onDelete(country)} className="icon-button shrink-0 text-red-500" aria-label={`Удалить ${country.name}`}><Trash2 size={17} /></button></div>)}</div></section></div></main>
}
...
function CountrySheet({ country, order, onClose, onSave, onError }) {
  ...
  return <Sheet title={country ? 'Редактировать страну' : 'Новая страна'} onClose={close}><form onSubmit={submit} className="space-y-4"><div className="grid grid-cols-[90px_1fr] gap-3"><label><span className="label">Флаг</span><input className="field text-center text-2xl" value={emoji} onChange={(event) => setEmoji(event.target.value)} maxLength={8} /></label><label><span className="label">Название</span><input autoFocus className="field" value={name} onChange={(event) => setName(event.target.value)} placeholder="Индонезия" maxLength={80} /></label></div><button disabled={saving} className="flex w-full items-center justify-center gap-2 rounded-[20px] bg-ink px-4 py-4 font-extrabold text-white shadow-[0_8px_22px_rgba(23,33,31,0.18)] dark:bg-mint-600">{saving && <LoaderCircle size={19} className="animate-spin" />} Сохранить</button></form></Sheet>
}
...
function EmptyState({ favorites, completed, onAdd }) {
  ...
  return <div className="rounded-[30px] border border-white bg-white/95 px-6 py-14 text-center shadow-[0_6px_24px_rgba(23,33,31,0.055)] dark:border-white/10 dark:bg-surface/95"><div className="mx-auto mb-3 grid h-12 w-12 place-items-center rounded-full bg-mint-50 text-mint-700 dark:bg-mint-500/15 dark:text-mint-200">{favorites ? <Heart size={22} /> : completed ? <Check size={22} /> : <MapPin size={22} />}</div><h2 className="font-extrabold">{title}</h2><p className="mt-1 text-sm text-muted">{description}</p>{!favorites && !completed && <button onClick={onAdd} className="mt-4 rounded-full bg-mint-50 px-4 py-2.5 text-sm font-bold text-mint-700 dark:bg-mint-500/15 dark:text-mint-200">Добавить место</button>}</div>
}
...
function AccessDenied({ username }) {
  return <main className="grid min-h-dvh place-items-center px-6 text-center"><div><div className="mx-auto mb-5 grid h-16 w-16 place-items-center rounded-full bg-white text-mint-700 shadow-soft dark:bg-surface dark:text-mint-200"><LockKeyhole size={28} /></div><h1 className="text-2xl font-extrabold">Доступ закрыт</h1><p className="mt-2 text-sm text-muted">{username ? `Аккаунт @${username} не добавлен.` : 'Откройте приложение внутри Telegram.'}</p></div></main>
}
