  ...
  return <div className="mx-auto min-h-dvh w-full max-w-2xl px-3 pb-[calc(6.75rem+env(safe-area-inset-bottom))] pt-[calc(40px+env(safe-area-inset-top))] sm:px-6 sm:pt-[calc(2rem+env(safe-area-inset-top))]">
    {section === 'settings' ? <SettingsScreen countries={countries} username={username} onAdd={() => setCountryModal({})} onEdit={(country) => setCountryModal(country)} onDelete={async (country) => {
      ...
      <header className="mb-3 grid gap-3 sm:mb-5 sm:flex sm:items-center sm:justify-between">
        ...
        {section === 'places' && <div className="grid w-full grid-cols-2 rounded-[20px] border border-white/80 bg-white/90 p-1 shadow-[0_4px_18px_rgba(23,33,31,0.06)] backdrop-blur sm:flex sm:w-auto"><button onClick={() => setStatus('all')} className={`min-h-10 rounded-2xl px-3 py-2 text-xs font-bold transition ${status === 'all' ? 'bg-ink text-white shadow-sm' : 'text-muted'}`}>Все</button><button onClick={() => setStatus('done')} className={`min-h-10 rounded-2xl px-3 py-2 text-xs font-bold transition ${status === 'done' ? 'bg-ink text-white shadow-sm' : 'text-muted'}`}>Выполненные</button></div>}
      </header>
...
function Sheet({ title, onClose, children, footer }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-900/30 backdrop-blur-md sm:items-center sm:p-4" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className="flex max-h-[calc(100dvh-env(safe-area-inset-top)-8px)] w-full max-w-lg flex-col overflow-hidden rounded-t-[34px] border border-white/70 bg-canvas shadow-[0_24px_70px_rgba(23,33,31,0.22)] sm:max-h-[94dvh] sm:rounded-[34px]"><div className="mx-auto mt-2.5 h-1.5 w-11 shrink-0 rounded-full bg-slate-300/90 sm:hidden" /><header className="flex shrink-0 items-center justify-between px-4 py-3 sm:px-5 sm:py-4"><h1 className="text-lg font-extrabold sm:text-xl">{title}</h1><button onClick={onClose} className="icon-button bg-white/70 shadow-sm" aria-label="Закрыть"><X size={21} /></button></header><div className="min-h-0 flex-1 overscroll-contain overflow-y-auto px-4 pb-4 sm:px-5 sm:pb-5">{children}</div>{footer && <footer className="shrink-0 border-t border-white/80 bg-white/[0.88] px-4 pb-[calc(0.75rem+env(safe-area-inset-bottom))] pt-3 backdrop-blur-xl sm:px-5 sm:pb-4">{footer}</footer>}</section></div>
}

function EmptyState({ favorites, onAdd }) {
  return <div className="rounded-[30px] border border-white bg-white/95 px-6 py-14 text-center shadow-[0_6px_24px_rgba(23,33,31,0.055)]"><div className="mx-auto mb-3 grid h-12 w-12 place-items-center rounded-full bg-mint-50 text-mint-700">{favorites ? <Heart size={22} /> : <MapPin size={22} />}</div><h2 className="font-extrabold">{favorites ? 'Здесь появится любимое' : 'Пока ничего нет'}</h2><p className="mt-1 text-sm text-muted">{favorites ? 'Нажмите на сердце у нужного места.' : 'Добавьте первое место в эту категорию.'}</p>{!favorites && <button onClick={onAdd} className="mt-4 rounded-full bg-mint-50 px-4 py-2.5 text-sm font-bold text-mint-700">Добавить место</button>}</div>
}
